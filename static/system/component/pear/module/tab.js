layui.define(['jquery', 'element'], function(exports) {
	...
	function closeEvent(option) {
		$(".layui-tab[lay-filter='" + option.elem + "']").on("click", ".layui-tab-close", function () {
			...
			console.log("关闭标签: " + layid);

			// 判断标签的 lay-id 是否包含指定数字
			if (layid == 75 || layid == 76) {
				// 如果包含指定数字，先执行关闭操作
				confirmAndExecute(option, layid);
			} else {
				// 否则直接关闭标签页
	...
	function confirmAndExecute(option, currentId) {
		layer.confirm('请确认当前是否关闭摄像头？', {icon: 3, title: '关闭摄像头'}, function (index) {
			$.ajax({
				type: "POST",
				url: "system/vehicle/info/stop_camera",
	...
	function menuEvent(option, index) {
		...
		$("#" + option.elem + "closeThis").click(function () {
			...
			if (currentTab.find("span").is(".able-close")) {
				var currentId = currentTab.attr("lay-id");
				if (currentId == 75 || currentId == 76) {
					confirmAndExecute(option, currentId);
				} else {
					tabDelete(option.elem, currentId, option.closeEvent, option);
				}
			} else {
		...
		$("#" + option.elem + "closeAll").click(function () {
			var tabtitle = $(".layui-tab[lay-filter='" + option.elem + "'] .layui-tab-title li");
			var hasSpecialTabs = false; // 用于标记是否存在包含特定数据的标签页

			$.each(tabtitle, function (i) {
				var layId = $(this).attr("lay-id");
				if (layId == 75 || layId == 76) {
					// 如果标签的 lay-id 包含指定数字，就执行关闭操作
					confirmAndExecute(option, layId);
					hasSpecialTabs = true;
				}
			});

			// 如果存在包含特定数据的标签页，不执行关闭全部操作
			if (!hasSpecialTabs) {
				$.each(tabtitle, function (i) {
					var layId = $(this).attr("lay-id");
					tabDelete(option.elem, layId, option.closeEvent, option);
